...
建立一個 [D1][cf-d1] 資料庫，這是 Cloudflare 提供的 SQLite 相容資料庫服務，Yang Chun Comment 主要使用它來存放留言資料。以下指令中的 `yangchun-comment-d1` 是這個資料庫的名稱，可以自行修改。關於 D1 的免費額度，請參考 [D1 Pricing][cf-d1-price]。如果你需要指定地區，可以使用 `--location` 或 `--jurisdiction`，詳細請參考 [Cloudflare 的文件](https://developers.cloudflare.com/workers/wrangler/commands/#d1-create)
...
首先是管理員登入密碼。Yang Chun Comment 使用 PBKDF2-HMAC-SHA256 作為密碼雜湊演算法，設定輸出長度為 32 bytes，並採用 600,000 次迭代（[OWASP 建議][owasp-pbkdf2]）。你可以使用下面的 Node.js 腳本來產生：
...
```txt "(Y/n)"
🌀 Creating the secrets for the Worker "yangchun-comment-be" 
✔ There doesn't seem to be a Worker called "yangchun-comment-be".
  Do you want to create a new Worker with that name and add secrets to it? (Y/n)
```
```txt
🌀 Creating new Worker "yangchun-comment-be"...
...
<Aside type="danger">
帶有 `SECRET_` 前綴的變數是敏感資訊，請務必妥善保管。實際上，一旦完成本節的步驟後，你就不需要（也不應該）再查看或保管這些密鑰。它們儲存在 Cloudflare Workers 的 Secrets 中且無法被讀取或導出。
</Aside>
...
你可以嘗試在瀏覽器中打開這個網址 `https://yangchun-comment-be.<YOUR_SUBDOMAIN>.workers.dev/rss/site/<RSS_SITE_PATH>` 來檢查 Worker 是否成功部署，這個是全站的 RSS Feed 網址，記得要把其中的 `<YOUR_SUBDOMAIN>` 和 `<RSS_SITE_PATH>` 替換成你的實際值，注意網址沒有尾斜線。如果成功的話你應該會看到一個類似下面的 XML 回應：
