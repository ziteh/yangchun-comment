...
建立一個 [D1][cf-d1] 資料庫，這是 Cloudflare 提供的 SQLite 相容資料庫服務，Yang Chun Comment 主要使用它來存放留言資料。以下指令中的 `yangchun-comment-d1` 是這個資料庫的名稱，可以自行修改。關於 D1 的免費額度，請參考 [D1 Pricing][cf-d1-price]。

```bash
pnpm exec wrangler d1 database create "yangchun-comment-d1" --binding "DB"
```

預計會得到類似下面的回應。你需要複製回應中的 `id`。

```txt "506ac68ae906458692g22933d3896af3"
...🌀 Creating database with title "yangchun-comment-d1"
✨ Success!
