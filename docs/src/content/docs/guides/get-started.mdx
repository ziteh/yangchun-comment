...
<Aside type="note" title="關於指紋識別">
目前 Yang Chun Comment 對於***一般留言者***的流量限制是採用工作量證明（Proof-of-Work）機制，未使用任何包含用戶個人資訊的識別方式。
...
建立一個 [D1][cf-d1] 資料庫，這是 Cloudflare 提供的 SQLite 相容資料庫服務，Yang Chun Comment 主要使用它來存放留言資料。以下指令中的 `yangchun-comment-db` 是這個資料庫的名稱，可以自行修改。關於 D1 的免費額度，請參考 [D1 Pricing][cf-d1-price]。

```bash
pnpm exec wrangler d1 database create "yangchun-comment-db" --binding "DB"
```

預計會得到類似下面的回應。你需要複製回應中的 `id`。

```txt "506ac68ae906458692g22933d3896af3"
...🌀 Creating database with title "yangchun-comment-db"
✨ Success!
...
[[d1_databases]]
binding = "DB"
id = "506ac68ae906458692g22933d3896af3" # <-- Replace with your D1 ID
...
```
...
```bash
pnpm exec wrangler d1 migrations apply "yangchun-comment-db" --remote
```
...
現在你的 Worker 應該會顯示失敗，這是預期的，因為還沒有上傳密鑰。
...
密鑰上傳完成後，檢查你的 Worker，它現在應該可以正常運作了！
