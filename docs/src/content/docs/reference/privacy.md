...
不保證系統設計符合包含但不限於 GDPR (General Data Protection Regulation) 在內的任何資料和隱私相關法規。這份文件僅用於在技術上說明系統的設計和運作方式。
:::
...
假名使用的詞庫不大（139 個形容詞和 549 個名詞，總詞組空間為 139\*549=76,311），刻意設計成容易發生碰撞，非常難以逆向回推使用者原始輸入的昵稱，因為可能性非常多。
...
對於***留言者***的流量限制是採用工作量證明（Proof-of-Work）機制，未使用任何包含用戶個人資訊（如 IP 位址、User-Agent 或 Cookie 等）的識別方式。
